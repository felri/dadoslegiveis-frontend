...
function JoyplotChart({ data, getDetails }: Props): JSX.Element {
  ...
  const init = (data: JoyplotData) => {
    ...
    const chartMargins = { top: 120, right: 50, bottom: 30, left: 40 };
    ...
    const chartWidth = rect ? rect.width - chartMargins.left - chartMargins.right : 0;
    ...
    const xScale = d3
      ...
      .range([chartMargins.left + 80, chartWidth - chartMargins.right - 20])
      .nice();
    ...
    // @ts-ignore
    const xAxis = (g) =>
      g.attr("transform", `translate(0, 80)`).call(
        d3
          .axisBottom(xScale)
          .ticks(chartWidth / data.dates.length)
          .tickSizeOuter(1)
      );
    // @ts-ignore
    const yAxis = (g) =>
      g
        .attr("transform", `translate(${chartMargins.left + 160}, 0)`)
    ...
    const yAxis2 = (g) =>
      g
        ...
        .call(d3.axisLeft(y2Scale).tickSize(0).tickPadding(4))
        .style("font-size", "13px")
        // @ts-ignore
        .call((g) => g.select(".domain").remove());
